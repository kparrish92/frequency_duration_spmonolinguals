...
span_model_df <- span_model %>% 
  as.data.frame()
hdi_span <- hdi(span_model_df$b_log_freq) 
...
eng_post <- as_tibble(eng_model) %>% select(int = b_Intercept, b = b_log_freq) %>% 
...
eng_data %>% 
  ggplot(., aes(x = log_freq, y = duration, color = length, shape = rate)) + 
    geom_point(pch = 20) + geom_abline(intercept = y_int_e, slope = fixef(eng_model)[2, 1], 
